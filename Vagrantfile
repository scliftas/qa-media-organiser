required_plugins = %w( vagrant-hostsupdater vagrant-host-shell vagrant-triggers )
...
Vagrant.configure("2") do |config|
    ...
    config.trigger.after :destroy do
        # Run after as before causes a delete before the machine is confirmed to be deleted
        info "Removing domain.txt"
        run  "rm -f domain.txt"
